...
1. `flutter-dev-guide/AGENTS.md`，再依任務讀其指定 guides 與 checklist。
2. 本功能已存在的完整 `spec.md`、`plan.md`、`tasks.md`；不可只根據摘要實作。若目前 workflow 正在建立其中一項工件，先讀取已存在工件，並在 progress 記錄待建立項目。
3. `agent-workflow/WORK_GRAPH.yaml` 中已存在的對應 task 節點與 `agent-workflow/progress.md`。尚未有節點時，由 `work-graph` 建立。
