...
selects project `allenljf-algorithm`, deploys the production Wasm bundle from
...
./gradlew :composeApp:wasmJsBrowserProductionWebpack -PapiBaseUrl=https://allenljf-algorithm.web.app
```
...
`https://allenljf-algorithm.web.app` CORS origin, and deploy Hosting. It must
...
The browser uses `/api/**` at `https://allenljf-algorithm.web.app`; the API's
